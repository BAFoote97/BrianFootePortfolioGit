...
public class Team2LockOnMe : MonoBehaviour {
   ...
   // public GameObject realIndic;
	...
	public GameObject outerDiamond;
	public GameObject innerDiamond;
    public bool isLockedOnByEnemy;
	public bool isLockedOnByPlayer;
	...
	void Update () {
        ...
        if (character.GetComponent<BasicAI>().dead == true)
        {
            isDead = true;
            isTargettedByPlayer = false;
            isLockedOnByPlayer = false;
            //Destroy(aiMarker);
		...
		if (isLockedOnByPlayer == true || isLockedOnByEnemy == true && character.GetComponent<BasicAI>().dead == false) {
            if (isLockedOnByPlayer == true)
            {
                outerDiamond.gameObject.SetActive(true);
                innerDiamond.gameObject.SetActive(true);
            }
            if (character.GetComponent<BasicAI>().genericEnemy == true)
            {
                character.GetComponent<BasicAI>().subtitleObject.GetComponent<DialogueScript>().GenericEnemyPlayerLockedOn();
            }
		...
		if (isLockedOnByPlayer == false) {
			outerDiamond.gameObject.SetActive(false);
			innerDiamond.gameObject.SetActive(false);
